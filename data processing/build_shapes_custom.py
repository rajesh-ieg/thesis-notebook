...
def test_process_func_pop(df_gadm_subset):
    # get subset by country code
...
##workflow for custom GADM shapes

    geodf_temp = gpd.read_file(
            "resources/shapes/gadm_shapes_custom.geojson", driver = "GeoJSON"
        )
    
    print(geodf_temp)
    geodf_temp["pop"] = 0.0
    newvar = test_process_func_pop(geodf_temp)
    add_gdp_data(newvar)
    newvar= newvar.set_index("GADM_ID")
    #save and overwrite the new custom shape with pop gdp
    save_to_geojson(newvar, out.gadm_shapes)
